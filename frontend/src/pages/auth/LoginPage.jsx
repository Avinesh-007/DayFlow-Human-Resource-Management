...
function AuthFrame({ title, subtitle, children }) { return <Box sx={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: { md: '1.1fr 1fr' }, bgcolor: 'background.default' }}><Box sx={{ display: { xs: 'none', md: 'flex' }, flexDirection: 'column', justifyContent: 'space-between', p: 8, color: '#fffaf0', bgcolor: 'primary.main', background: 'linear-gradient(145deg, #10212b 0%, #1b4b55 70%, #ef765f 150%)' }}><BrandMark light /><Typography variant="h2" sx={{ maxWidth: 520, fontSize: 'clamp(2.8rem, 5vw, 5rem)', lineHeight: 1.02 }}>Make room for better work.</Typography><Typography color="#c8d9d6">People, presence, and progress in one calm place.</Typography></Box><Box sx={{ display: 'grid', placeItems: 'center', p: { xs: 3, sm: 6 }, background: 'radial-gradient(circle at 80% 10%, rgba(239,118,95,.12), transparent 28%), #f4f1eb' }}><Paper elevation={0} sx={{ p: { xs: 3, sm: 5 }, width: '100%', maxWidth: 440, bgcolor: 'transparent' }}><Box sx={{ mb: 4, display: { xs: 'block', md: 'none' } }}><BrandMark /></Box><Typography variant="h3" sx={{ mb: 1 }}>{title}</Typography><Typography color="text.secondary" sx={{ mb: 4 }}>{subtitle}</Typography>{children}</Paper></Box></Box>; }
