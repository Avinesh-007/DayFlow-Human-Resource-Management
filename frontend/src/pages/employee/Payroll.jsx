...
export default function Payroll() {
  ...
  const load = async () => { setLoading(true); setError(''); try { const response = await getMyPayroll({ page: 0, size: 10 }); setPage(response.data); } catch (requestError) { setError(getApiMessage(requestError, 'Unable to load payroll information.')); } finally { setLoading(false); } };
  useEffect(() => { load(); }, []);
